...
Then('the system should display no matches message', async function (this:CustomWorld) {
  const text = await this.exploretraining.getTextoferrorText();
  expect(text).toContain("No trainings available yet")
});

When('the user enters a partial course name in the search bar field', async function (this:CustomWorld) {
